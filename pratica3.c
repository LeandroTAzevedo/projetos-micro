...
sbit LCD_D7_Direction at TRISB3_bit; // Direcao do fluxo de dados do pino RB3
 ...
 unsigned char Tensao[10]; // Display LCD - tipo char - int 8 bits
 char Temp[8];
 int Va;
 int Ta;
 int Vmax = 500;
...
void main() {
 ...
 while(1) {
  ...
  Tensao[2] = (Va/10)%10 + '0';
  Tensao[3] = (Va/1)%10 + '0';
  LCD_Out(1,9,Tensao); // Apresenta o texto no LCD em determinada posicao
  ...
  Temp[3] = (Ta)%10 + '0';
  Temp[4] = 0;
  Lcd_Out(2,9,Temp); // Apresenta o texto no LCD em determinada posicao
  Delay_ms(20); // Delay para permitir a atualizacao do LCD
